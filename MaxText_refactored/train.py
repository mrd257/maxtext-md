...
def setup_mesh_and_model(config):
  ...
  # Mesh definition
  # create_device_mesh returns what jax.experimental.mesh_utils.create_device_mesh returns:  A np.ndarray of JAX devices with mesh_shape as its shape
  devices_array = max_utils.create_device_mesh(config)

  mesh = Mesh(devices_array, config.mesh_axes) #mesh axes ['data', 'fsdp', 'fsdp_transpose', 'sequence', 'tensor', 'autoregressive'] don't love this.  this needs to be in the same order as what is defiined in max_utils.create_device_mesh, so should probably just be returned by that function
  """
  Mesh:
  devices (ndarray) – A NumPy ndarray object containing JAX device objects).
...
def setup_train_loop(config):
  ...
  # Model setup
  """
  In this model setup embeddings are initilized 
  Then passed to Decoder 
