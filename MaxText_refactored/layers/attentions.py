...
class AttentionOp(nn.Module):
  ...
  def apply_attention(self, query: Array, key: Array, value: Array, decoder_segment_ids: Array | None, model_mode: str):
    ...
    # Supported attention kernels: autoselected, dot_product, flash, cudnn_flash_te
    if (
...
class Attention(nn.Module):
  ...
  def __call__(
    ...
    """
    # apply projection.
    if self.config.fused_qkv: # False
    ...
    attention_op = AttentionOp(
        mesh=self.mesh,
        attention_kernel=self.attention_kernel, # options are autoselected, dot_product, flash, cudnn_flash_te. the base choice is autoselected
