...
def create_device_mesh(config, devices=None):
  ...
  # Devices=None when called from train.py
  if devices is None:
    # Returns a List of Device subclasses. A list of jaxlib.xla_extension.Device
    devices = jax.devices()

  num_devices = len(devices)
  num_slices = config.num_slices # this is auto configured somewhere and the value in the base.yml(-1) is changed
  ...
  #presumably ordered by network intensity
  ici_parallelism = [
  ...
  # Looks for any -1 values and fills the value. Only if there is only one -1 value.
