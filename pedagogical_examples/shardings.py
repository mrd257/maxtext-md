...
def main_original(_argv: Sequence[str]) -> None:
...
def main(argv: Sequence[str]) -> None:
  # Note that pyconfig initializes the jax distributed system 
  ...
  # Create device mesh
  # Assumes single slice and ici parallelism only
  devices_array = mesh_utils.create_device_mesh(ici_parallelism)
  print(f"Decided on mesh shape: {devices_array}")
  ...
  BATCH = len(jax.devices()) * config.batch_size
  D_EMB = 8192
  D_FF = 4 * D_EMB
  NUM_LAYERS = 10
